...
def get_connection(): 
    try: 
        ...
        print("Couldn't connect to MySQL. Please.") 
        print("Error:", e) 
        return None 
...
def create_student(): 
    ...
    if not conn: return 
    cur = conn.cursor() 
    cur.execute("INSERT INTO schoolstudent (name, age) VALUES (%s, %s)", (name, age)) 
...
def read_schoolstudents(): 
    conn = get_connection() 
    if not conn: return 
    cur = conn.cursor() 
    cur.execute("SELECT id, name, age FROM schoolstudent ORDER BY id")
 
    rows = cur.fetchall() 
    if not rows: 
        print("No schoolstudent yet. Try option 1 to add one.") 
    else: 
        print("\n--- Students ---") 
        for row in rows: 
...
def update_student(): 
    ...
    if not conn: return 
    cur = conn.cursor() 
    cur.execute("UPDATE schoolstudent SET name=%s, age=%s WHERE id=%s", (new_name, new_age, id_text))
    conn.commit() 
    if cur.rowcount == 0: 
...
def delete_student(): 
    ...
    if not conn: return 
    cur = conn.cursor() 
    cur.execute("DELETE FROM schoolstudent WHERE id=%s", (id_text,)) 
    conn.commit() 
    if cur.rowcount == 0: 
        print("No student with that ID was found.") 
    else: 
...
def main(): 
    ...
    while True: 
        ...
        print("2) Read (show all schoolstudent)") 
