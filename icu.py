...
for readings in patient_sensor_stream(patients): 
    for reading in readings: 
        ...
        # --- Alerts --- 
        alerts = [] 
        if hr < NORMAL_RANGES["heart_rate"][0] or hr > NORMAL_RANGES["heart_rate"][1]:alerts.append(f"Heart Rate abnormal: {hr} bpm")  
        if temp < NORMAL_RANGES["t emperature"][0] or temp > NORMAL_RANGES["temperature"][1]: alerts.append(f"Temperature abnormal: {temp} °F")
        if ox < NORMAL_RANGES["oxygen_level"][0]:  alerts.append(f"Oxygen Level low: {ox}%") 
        if alerts: 
